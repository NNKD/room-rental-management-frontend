...
export default function UserApartmentManagement() {
    ...
    const headers: TableHeader<UserApartmentDTO>[] = [
        {name: "Tên phòng", slug: "name", center: true},
        {name: "Loại phòng", slug: "type", center: true},
        {name: "Ngày thuê", slug: "startDate", center: true},
        {name: "Ngày hết hạn", slug: "endDate", center: true},
    ]
    ...
    const handleGetApartments = async () => {
        ...
        try {
            ...
            if (response.status === 200 && response.data.status == 'success' && response.data.statusCode == 200) {
                ...
                setApartments(response.data.data.map((a: UserApartmentDTO) => ({
                    ...a,
                    name: <Link to={`${a.slug}`} className="underline text-blue-500" data-sort={a.name}>{a.name}</Link>,
    ...
    return (
        <div className="h-full flex flex-col overflow-hidden">
            <h1 className="mb-8 font-bold text-2xl mt-16 lg:mt-0">Quản lý căn hộ đang thuê</h1>
