...
export default function ApartmentPriceService() {
    ...
    const handleGetServices = async () => {
        try {
            ...
            setMessage(axiosError.response?.data?.message || "Đã có lỗi xảy ra khi lấy danh sách dịch vụ");
    ...
    const confirmDeleteService = async () => {
        ...
        try {
            ...
            const axiosError = error as AxiosError<{ message?: string }>;
            setMessage(axiosError.response?.data?.message || "Đã có lỗi xảy ra khi xóa dịch vụ");
            setType(NoticeType.ERROR);
        } finally {
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        try {
            let response;
            if (editingService) {
                response = await axios.put(`${envVar.API_URL}/services/${editingService.id}`, payload, {
                    headers: { Authorization: `Bearer ${localStorage.getItem("jwt_token")}` },
                });
            } else {
                response = await axios.post(`${envVar.API_URL}/services`, payload, {
                    headers: { Authorization: `Bearer ${localStorage.getItem("jwt_token")}` },
                });
            }
            if (response.status === 200 && response.data.status === "success") {
                setMessage(editingService ? "Cập nhật dịch vụ thành công" : "Thêm dịch vụ thành công");
                setType(NoticeType.SUCCESS);
            ...
            const axiosError = error as AxiosError<{ message?: string }>;
            setMessage(axiosError.response?.data?.message || "Đã có lỗi xảy ra khi lưu dịch vụ");
            setType(NoticeType.ERROR);
        }
    };
