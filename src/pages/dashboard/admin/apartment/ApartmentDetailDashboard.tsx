...
export default function ApartmentDetailDashboard() {
    ...
    const [types, setTypes] = useState<ApartmentTypeDTO[]>([]);
    const [visibleAmount, setVisibleAmount] = useState(3);
    const [mode, setMode] = useState<"read" | "update" | "add">("read");
    const [imageZoom, setImageZoom] = useState("");
    const inputNameRef = useRef<HTMLInputElement | null>(null);
    const { setMessage, setType } = useNotice();
    const [durationMonth, setDurationMonth] = useState(1);
    ...
    const defaultApartmentDetail: ApartmentDTO = {
        ...
        discounts: [
            ...
            { discount_percent: 0, duration_month: 3 },
            { discount_percent: 0, duration_month: 6 },
            { discount_percent: 0, duration_month: 12 },
        ],
        images: [],
        information: {
            id: 0,
    ...
    useEffect(() => {
        const handleResize = () => {
            if (window.innerWidth < 768) setVisibleAmount(1);
            else if (window.innerWidth < 1024) setVisibleAmount(2);
            else setVisibleAmount(3);
        };
    ...
    useEffect(() => {
        if (mode === "update" && inputNameRef.current) {
            inputNameRef.current.focus();
        }
    }, [mode]);
    ...
    const handleGetDetail = async () => {
        try {
            ...
            setMessage(axiosError.response?.data?.message || "Không thể lấy chi tiết căn hộ");
            setType(NoticeType.ERROR);
        }
    };
    ...
    const handleGetAllStatus = async () => {
        try {
            ...
            const axiosError = error as AxiosError<{ message?: string }>;
            setMessage(axiosError.response?.data?.message || "Không thể lấy danh sách trạng thái");
            setType(NoticeType.ERROR);
        }
    };
    ...
    const debounceCheckSlug = useMemo(() => debounce((slugName: string) => handleCheckSlug(slugName), 500), []);

    const handleChangeField = (field: keyof ApartmentDTO, value: unknown) => {
        setApartmentDetail((prev) => ({ ...prev, [field]: value }));
    };

    const handleChangeFieldInformation = (field: keyof ApartmentDTO["information"], value: unknown) => {
        setApartmentDetail((prev) => ({
            ...prev,
    ...
    const handleChangeFieldDiscount = (value: string) => {
        setApartmentDetail((prev) => ({
            ...prev,
            discounts: prev.discounts.map((d) =>
                d.duration_month === durationMonth ? { ...d, discount_percent: Number(value) } : d,
            ),
        }));
    };
    ...
    const handleDeleteImg = (url: string) => {
        ...
        if (publicId) {
            setImagesDeleteCloudinary((prev) => [...prev, { url: publicId }]);
        }
    };

    const handleAddOrUpdateApartment = async () => {
        if (!apartmentDetail.name || !apartmentDetail.slug || !apartmentDetail.brief || !apartmentDetail.description) {
        ...
        try {
            ...
            if (response.status === 200 && response.data.status === "success" && response.data.statusCode === 200) {
                setType(NoticeType.SUCCESS);
                setMessage(response.data.message || "Lưu căn hộ thành công");
                setMode("read");
            }
        } catch (error: unknown) {
            const axiosError = error as AxiosError<{ message?: string }>;
            setMessage(axiosError.response?.data?.message || "Không thể lưu căn hộ");
            setType(NoticeType.ERROR);
        } finally {
            setLoading(false);
        }
    ...
    const handleCheckSlug = async (slugName: string) => {
        try {
            ...
            const axiosError = error as AxiosError<{ message?: string }>;
            setMessage(axiosError.response?.data?.message || "Không thể kiểm tra slug");
            setType(NoticeType.ERROR);
        }
    };

    const handleDeleteImgOnCloudinary = async () => {
        if (imagesDeleteCloudinary.length === 0) return;
        try {
            await axios.post(`${envVar.API_URL}/cloudinary/delete-images`, imagesDeleteCloudinary, {
                headers: { Authorization: `Bearer ${token}` },
            });
        } catch (error: unknown) {
            console.error("Error deleting images on Cloudinary:", error);
        }
    };
    ...
    return (
        <div className="overflow-auto h-fit max-h-full w-full rounded shadow-[0_0_3px_2px_#ccc] mt-16 lg:mt-0 lg:max-h-full">
                                ...
                                <div
                                    className={`px-4 py-2 text-base ${
                                        mode === "read"
                                            ...
                                            : "bg-zinc-300 hover:bg-lightGreenHover cursor-pointer transition-all duration-300 ease-in-out"
                                    }`}
                                    onClick={() => {
                                        handleSave();
                                        setMode("read");
                        ...
                        <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
                            <div className="flex items-center justify-between border p-2 rounded w-full">
                                ...
                                <select
                                    ...
                                    onChange={(e) => handleChangeField("status", { id: Number(e.target.value) })}
                                >
                                    {statuses.map((s) => (
                            ...
                            <div className="flex items-center justify-between border p-2 rounded w-full">
                                ...
                                <select
                                    ...
                                    onChange={(e) => handleChangeField("type", { id: Number(e.target.value) })}
                                >
                                    {types.map((t) => (
                        ...
                        <div className="flex flex-col lg:flex-row gap-6 items-center justify-between">
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/3">
                                ...
                                <select
                                    className="px-2 py-1 border-2 rounded"
                                    disabled={mode === "read"}
                                    value={apartmentDetail.hot || 0}
                                    onChange={(e) => handleChangeField("hot", Number(e.target.value))}
                                >
                            ...
                            <div className="flex items-center border p-2 rounded w-full lg:w-[12%]">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("floor", Number(e.target.value))}
                                    min={0}
                                    max={20}
                                />
                            </div>
                        ...
                        <div className="flex flex-col lg:flex-row items-center justify-center gap-6">
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/4">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("height", Number(e.target.value))}
                                    min={0}
                                />
                            </div>
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/4">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("width", Number(e.target.value))}
                                    min={0}
                                />
                            </div>
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/4">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("balcony", Number(e.target.value))}
                                    min={0}
                                />
                            </div>
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/4">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("terrace", Number(e.target.value))}
                                    min={0}
                                />
                            </div>
                        </div>
                        <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/4">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("bedrooms", Number(e.target.value))}
                                    min={0}
                                />
                            </div>
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/4">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("kitchens", Number(e.target.value))}
                                    min={0}
                                />
                            </div>
                            <div className="flex items-center justify-between border p-2 rounded w-full lg:w-1/4">
                                ...
                                <input
                                    ...
                                    disabled={mode === "read"}
                                    onChange={(e) => handleChangeFieldInformation("bathrooms", Number(e.target.value))}
                                    min={0}
                                />
                            </div>
                        </div>
                            ...
                            <div className="flex items-center justify-around gap-6 select-none">
                                ...
                                <div
                                    className={`w-[30px] h-[30px] md:w-[46px] md:h-[46px] flex items-center justify-center text-white rounded-full flex-shrink-0 ${
                                        indexCarousel === Math.max(0, apartmentDetail.images.length - visibleAmount)
                                            ? "bg-lightBlue pointer-events-none"
                                            : "bg-lightGreen cursor-pointer hover:bg-lightGreenHover transition-all duration-300 ease-in-out"
                                    }`}
                                    onClick={() => handleCarousel(1)}
